...
def distribution():
    ...
    print(f'Received order from the kitchen. orderId: {order["order_id"]}') 
...
class Waiter(threading.Thread):
    ...
    def search_order(self):
        try:
            ...
            table_id = next((i for i, table in enumerate(config.TABLES) if table['id'] == order['table_id']), None)
            print(f'{threading.current_thread().name} picked up the order with Id: {order["id"]} | priority: {order["priority"]} | items: {order["items"]} from table {order["table_id"]}')
            config.TABLES[table_id]['state'] = config.TABLE_STATE_2
            payload = dict({
                'order_id': order['id'],
    ...
    def serve_order(self, order_to_serve):
        ...
        if req_order is not None and req_order['items'].sort() == order_to_serve['items'].sort():
            ...
            config.TABLES[table_id]['state'] = config.TABLE
            ...
            config.ORDER_STARS.append(order_stars)
            sum_stars = sum(feedback['star'] for feedback in config.ORDER_STARS)
            avg = float(sum_stars / len(config.ORDER_STARS))

            served_order = {**order_to_serve, 'serving_time': order_serving_time, 'status': config.ORDER_SERVED}
...
class Costumers(threading.Thread):
    ...
    def generate_order():
        ...
        if table_id is not None:
            ...
            idxs = [table for table in config.TABLES if table['state'] == config.TABLE_STATE_0]
            if len(idxs):
                rand_idx = random.randrange(len(idxs))
                config.TABLES[rand_idx]['state'] = config.TABLE_STATE_0
